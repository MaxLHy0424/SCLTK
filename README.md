...
- `crack_helper`：破解时执行的可执行文件的相对路径（**不支持绝对路径，不支持传参，路径分隔符推荐使用反斜杠**）。
- `restore_helper`：恢复时执行的可执行文件的相对路径（**不支持绝对路径，不支持传参，路径分隔符推荐使用反斜杠**）。
...
`private-key-id` 为签名 SCLTK 时所需的**私钥**。
