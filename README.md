...
> 这里针对 “学生机房管理助手” 做补充说明。
>
> 该软件在运行时会启动一个**进程名完全随机**的**守护进程**。当主进程被终止时，守护进程将尝试重新启动主进程。如果恢复失败，守护进程则会*蓝屏*（非 Windows BSOD）警告用户。由于性能问题，**守护进程并不在破解/恢复规则中**。所以，破解 “学生机房管理助手” 后，务必尽快使用[工具箱](#4-工具箱)中提供的的 `> 终止 "学生机房管理助手" 守护进程` 终止守护进程。
...
其中 `/path/to/msys2` 为 msys2 的安装路径（以 `/` 作为路径分隔符，禁止使用英文半角双引号包裹，如 `C:/dev_tools/msys2`，下同），`/path/to/pwsh` 为 PowerShell（仅支持 PowerShell 7 及以上版本）的安装路径（可通过命令 `where.exe pwsh.exe` 查询）。
