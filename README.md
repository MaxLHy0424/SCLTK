...
> 这里针对 “机房管理助手” 做补充说明。
>
> 该软件在运行时会启动一个**进程名完全随机**的**守护进程**。当主进程被终止时，守护进程将尝试重新启动主进程。如果恢复失败，守护进程则会*蓝屏*（非 Windows BSOD）警告用户。由于性能问题，**守护进程并不在破解/恢复规则中**。所以，破解 “机房管理助手” 前，务必使用[工具箱](#4-工具箱)中提供的的 `> 终止 "机房管理助手" 守护进程` 终止守护进程。
