...
**2025-11-23（UTC+08:00）** 从今起，SCLTK 暂时进入维护阶段，暂停新功能开发，在此深感抱歉。
...
| SCLTK v7 | [v7.0.1](https://github.com/MaxLHy0424/SCLTK/releases/tag/v7.0.1)           | ✅ 基线开发 | 2025-11-10/..         |
...
- **一键重启资源管理器**\
`> 重启资源管理器`
- **停止 `TDNetFilter` 与 `TDFileFilter` 来解除极域电子教室的网络限制与文件访问限制**\
`> 解除极域电子教室网络限制与文件访问限制`
- **通过修改注册表来恢复 USB 设备访问**\
`> 恢复 USB 设备访问`
...
每个控制软件有独立的破解/恢复选项，可根据需求执行。可以通过点击 `> 全部执行` 一次性执行所有内建规则和自定义规则。

当启用特定选项（详见 [3.1.1 破解与恢复](#31-破解与恢复)）时，破解/恢复时将映像劫持可执行文件，禁用相关服务。

点击 `> 自定义` 将执行自定义规则，配置自定义规则请参阅 [3.2 自定义规则](#32-自定义规则)。

> [!NOTE]
> 这里针对 “极域电子教室” 做补充说明。
>
> 由于未知原因，在破解时无法直接停止 `TDNetFilter` 与 `TDFileFilter` 两个服务，这可能无法解除网络限制与文件访问限制。如有需要，可使用[工具箱](#4-工具箱)中提供的 `> 解除极域电子教室网络限制与文件访问限制`。

> [!NOTE]
> 这里针对 “学生机房管理助手” 做补充说明。
>
> 该软件在运行时会启动一个**进程名完全随机**的**守护进程**。当主进程被终止时，守护进程将尝试重新启动主进程。如果恢复失败，守护进程则会*蓝屏*（非 Windows BSOD）警告用户。由于性能问题，**守护进程并不在破解/恢复规则中**。所以，破解 “学生机房管理助手” 前，务必使用[工具箱](#4-工具箱)中提供的的 `> 终止 "学生机房管理助手" 守护进程` 终止守护进程。
...
然后，使用 git 克隆本仓库到本地，在仓库本地目录下使用 PowerShell 运行 `.\make_env.ps1`，这将会创建 `env.mk` 文件，文件内容如下：
...
其中 `/path/to/msys2` 为 msys2 的安装路径（以 `/` 作为路径分隔符，禁止使用英文半角双引号包裹，如 `C:/dev_tools/msys2`），`/path/to/pwsh` 为 PowerShell（仅支持 PowerShell 7 及以上版本）的安装路径（可通过命令 `where.exe pwsh.exe` 查询，以 `/` 作为路径分隔符，允许使用英文半角双引号包裹，如 `"C:/Program Files/PowerShell/7-preview/pwsh.exe"`）。
...
- [lzh173](https://github.com/lzh173) 提供 “学生机房管理助手” 逆向工程后的部分代码。