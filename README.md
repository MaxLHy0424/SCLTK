...
- **尝试重新启用部分被禁用的操作系统组件**\
`> 恢复操作系统组件`
- **通过修改注册表来恢复 USB 存储设备访问**\
`> 恢复 USB 存储设备访问`
- **重置防火墙规则，重置 Hosts，刷新 DNS 缓存**\
`> 重置部分网络设置`
...
当启用特定选项（详见 [3.1.1 破解与恢复](#31-破解与恢复)）时，破解/恢复时将映像劫持进程，禁用相关服务。
...
通常，这是由于控制软件置顶窗口和 SCLTK 发生冲突导致的。这时，可以尝试按下 Win + D 返回桌面，再次尝试点击。
...
`/path/to/msys2` 为 msys2 的安装路径（以 `/` 作为路径分隔符，禁止使用英文半角双引号包裹，如 `C:/dev_tools/msys2`）。

`/path/to/pwsh` 为 PowerShell（仅支持 PowerShell 7 及以上版本）的安装路径（可通过命令 `where.exe pwsh.exe` 查询，以 `/` 作为路径分隔符，允许使用英文半角双引号包裹，如 `"C:/Program Files/PowerShell/7-preview/pwsh.exe"`）。

`/path/to/gpg` 为 GnuPG 命令行工具的安装路径（可通过命令 `where.exe gpg.exe` 查询，以 `/` 作为路径分隔符，允许使用英文半角双引号包裹，如 `"C:/Program Files/GnuPG/bin/gpg.exe"`）。

`key-id` 为签名 SCLTK 时所需的密钥 ID。
