...
auto main() -> int
{
    ...
    ui.add_back( "                    [ 主  页 ]\n\n" )
      ...
      .add_back( "" )
      .add_back( " > 自定义 ", std::bind_front( core::execute_rules, std::cref( core::custom_rules ) ) );
    for ( const auto& rule : core::builtin_rules ) {
        ui.add_back( std::format( " > {} ", rule.shown_name ), std::bind_front( core::execute_rules, std::cref( rule ) ) );
    }
