...
namespace cpp_utils
{
    ...
    inline auto dynamic_assert(
      ...
      std::stacktrace trace = std::stacktrace::current() ) noexcept( has_exceptions )
    {
        ...
        try {
            ...
            if ( std::forward< F >( func )() == false ) [[unlikely]] {
                print_without_formatting( stderr, make_log( "assertion failed!"sv, src_location, std::move( trace ) ) );
                std::terminate();
            }
#ifdef CPP_UTILS_MACRO_HAS_EXCEPTIONS
