...
namespace cpp_utils
{
    ...
    class proc_trace final
    {
      private:
        DWORD pid;
        HANDLE proc;
        auto get_proc_cpu_usage_( ULONGLONG& prev_process_time, ULONGLONG& prev_system_time ) const noexcept
        {
            if ( !proc ) {
                return -1.0;
            }
            FILETIME proc_create, proc_exit, proc_kernel, proc_user;
            if ( !GetProcessTimes( proc, &proc_create, &proc_exit, &proc_kernel, &proc_user ) ) {
                return -1.0;
            }
            ...
            const auto current_proc_time{
              ( static_cast< ULONGLONG >( proc_user.dwHighDateTime ) << 32 | proc_user.dwLowDateTime )
              + ( static_cast< ULONGLONG >( proc_kernel.dwHighDateTime ) << 32 | proc_kernel.dwLowDateTime ) };
            const auto current_sys_time{
              ( static_cast< ULONGLONG >( sys_kernel.dwHighDateTime ) << 32 | sys_kernel.dwLowDateTime )
              + ( static_cast< ULONGLONG >( sys_user.dwHighDateTime ) << 32 | sys_user.dwLowDateTime ) };
            double cpu_usage{ -1.0 };
            if ( prev_process_time != 0 && prev_system_time != 0 ) {
                const auto proc_diff{ current_proc_time - prev_process_time };
                const auto sys_diff{ current_sys_time - prev_system_time };
                if ( sys_diff > 0 ) {
                    cpu_usage = static_cast< double >( proc_diff ) / sys_diff * 100.0;
                }
            }
            prev_process_time = current_proc_time;
            prev_system_time  = current_sys_time;
            return cpu_usage;
        }
      public:
        struct result final
        {
            SIZE_T working_set_size_bytes;
            SIZE_T pagefile_usage_bytes;
            ULONGLONG read_ops_count;
            ULONGLONG write_ops_count;
            ULONGLONG read_bytes_count;
            ULONGLONG write_bytes_count;
            DWORD page_faults;
            DWORD thread_count;
            DWORD priority;
            double cpu_usage;
        };
        auto get() const noexcept -> std::expected< result, std::string_view >
        {
            if ( proc == nullptr ) {
                return std::unexpected{ "unable to open proc handle" };
            }
            result res{};
            PROCESS_MEMORY_COUNTERS pmc;
            if ( !GetProcessMemoryInfo( proc, &pmc, sizeof( pmc ) ) ) {
                return std::unexpected{ "unable to get memory info" };
            }
            res.working_set_size_bytes = pmc.WorkingSetSize;
            res.pagefile_usage_bytes   = pmc.PagefileUsage;
            IO_COUNTERS io;
            if ( !GetProcessIoCounters( proc, &io ) ) {
                return std::unexpected{ "unable to get I/O info" };
            }
            res.read_ops_count    = io.ReadOperationCount;
            res.write_ops_count   = io.WriteOperationCount;
            res.read_bytes_count  = io.ReadTransferCount;
            res.write_bytes_count = io.WriteTransferCount;
            res.priority          = GetPriorityClass( proc );
            const auto snapshot{ CreateToolhelp32Snapshot( TH32CS_SNAPTHREAD, 0 ) };
            if ( snapshot == INVALID_HANDLE_VALUE ) {
                return std::unexpected{ "unable to create thread snapshot" };
            }
            THREADENTRY32 te32;
            ...
            ULONGLONG prev_proc_time{ 0 }, prev_sys_time{ 0 };
            get_proc_cpu_usage_( prev_proc_time, prev_sys_time );
            using namespace std::chrono_literals;
            std::this_thread::sleep_for( 50ms );
            double cpu_usage{ get_proc_cpu_usage_( prev_proc_time, prev_sys_time ) };
            if ( cpu_usage < 0 ) {
                return std::unexpected{ "unable to get cpu usage" };
            }
            res.cpu_usage = cpu_usage;
            return res;
        }
        auto operator=( const proc_trace& ) -> proc_trace& = delete;
        auto operator=( proc_trace&& ) -> proc_trace&      = delete;
        proc_trace( const DWORD pid ) noexcept
          : pid{ pid }
          , proc{ OpenProcess( PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, pid ) }
        { }
        proc_trace( const proc_trace& ) = delete;
        proc_trace( proc_trace&& )      = delete;
        ~proc_trace() noexcept
        {
            if ( proc ) {
                CloseHandle( proc );
    ...
    template < typename T >
    auto output_proc_trace_info( T& output_src, const proc_trace::result& trace_result )
    {
        thread_local const std::flat_map< DWORD, const char* > priority_dict{
        ...
        std::print( output_src, "Working Set Size: {} bytes\n", trace_result.working_set_size_bytes );
        std::print( output_src, "Pagefile Usage: {} bytes\n", trace_result.pagefile_usage_bytes );
        std::print( output_src, "Read Operations Count: {}\n", trace_result.read_ops_count );
        std::print( output_src, "Write Operations Count: {}\n", trace_result.write_ops_count );
        std::print( output_src, "Read Bytes Count: {}\n", trace_result.read_bytes_count );
        std::print( output_src, "Write Bytes Count: {}\n", trace_result.write_bytes_count );
        std::print( output_src, "Page Faults: {}\n", trace_result.page_faults );
        std::print( output_src, "Thread Count: {}\n", trace_result.thread_count );
        std::print(
          output_src, "Priority: {}\n",
          priority_dict.contains( trace_result.priority ) ? priority_dict.at( trace_result.priority ) : "<unknown>" );
        std::print( output_src, "CPU Usage: {:.2f}%\n", trace_result.cpu_usage );
    }
#else
